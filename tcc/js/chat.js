  ...
  if (token) {
    try {
      ...
      fetch(`https://tcc-production.up.railway.app/api/starvingless/post/v1/pt/list?size=${listSize}`, requestOptions)
      .then(response => response.json())
      .then(post => {
        console.log(post);
        ...
        userElements.forEach(userElement => {
          userElement.addEventListener('click', () => {
            ...
            const postDetailsUrl = `https://tcc-production.up.railway.app/api/starvingless/post/v1/pt/id/${postId}`;
        ...
        $('.send_message').on('click', function() {
          ...
          const postEnvioUrl = `https://tcc-production.up.railway.app/api/starvingless/post/v1/pt/create`;
          fetch(postEnvioUrl, {
            method: "POST",
            headers: headers,
